...
def separar_caixa_entre_animais(img, resultados):
    ...
    if len(animais['lobo']) > 1:
        x = animais['lobo'][0][0::2]+animais['lobo'][1][0::2]
        y = animais['lobo'][0][1::2]+animais['lobo'][1][1::2]
    else:
        x = animais['lobo'][0][0::2]
        y = animais['lobo'][0][1::2]
    minX = min(x)
    maxX = max(x)
    minY = min(y)
    maxY = max(y)
...
def checar_perigo(image, animais):
    ...
    thickness = 2
    j = 0
    while (j < len(animais['vaca'])):
        if calcula_iou(animais['vaca'][j], [minX, minY, maxX, maxY]) >= 0.1:
            cv2.putText(image, 'Perigo!', org, font, fontScale, colorR, thickness, cv2.LINE_AA)
        else:
            cv2.putText(image, 'Tranquilo.', org, font, fontScale, colorB, thickness, cv2.LINE_AA)
        j += 1